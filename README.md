...
起動後はメニューバー常駐アプリとして動作し、対応ファイルは Finder の右クリックから `このアプリで開く` で要約できます。対応アプリでは、選択テキストの右クリック `サービス` からも要約でき、PDF や画像は Finder の右クリック `サービス` から OCR テキスト抽出や要約コピーもできます。右クリック時に AI 機能だと分かりやすいよう、サービス名は `DropSort AIで...` に統一しています。現在の preview は安定運用を優先して、常駐監視は停止しています。Quick Sort と右クリック操作が主導線です。

公開名と GitHub リポジトリ名は `DropSort` / `dropsort` に統一しています。一部の内部識別子は移行中です。

## 最初に試してほしいこと

1. X や記事の長文を選択して `サービス > DropSort AIで選択テキストを要約`
2. その画面をスクリーンショットして、Finder で `サービス > DropSort AIでOCRテキストをコピー`
3. 同じファイルで `サービス > DropSort AIでファイルを要約してコピー`
4. 最後にメニューバーから `Quick Sort Desktop` か `Quick Sort Downloads`

これで「読む」「抜く」「まとめる」「片付ける」が、全部ローカルでつながって見えます。
...
- Desktop / Downloads の整理候補を手動でレビュー
- スクリーンショットや PDF を右クリックから要約
- クリップボードの内容を手動で要約
...
- 現在の preview では安定運用を優先し、常駐監視は停止中
- Quick Sort と右クリック操作が主導線
